...
cleanFiles <- function(path, city, store){
  ...
  colnames(k) <- if_else( 
    ...
    stringr::str_c( str_sub(nm, 1, 10), str_sub(nm, 12),store,"evaluation", sep = "_"   ) , nm
    #if_else( grepl( "^HOBB_\\d__\\d\\.\\d+|^HOUS_\\d__\\d\\.\\d+|^FOOD_\\d__\\d\\.\\d+", nm), #\\d+{2,3}
    #        stringr::str_c( str_sub(nm, 1, 6), store, str_sub(nm, 9), "evaluation", sep = "_"), 
    #       stringr::str_c( str_sub(nm, 1, 5), store, 
    #                      str_sub(nm, 7), "evaluation", sep = "_"   ) 
    #)
  )
  k
}
...
stores_evaluation <- store_data_evaluation %>% 
...
head(stores_evaluation[,1:2])

#write
write_csv(stores_evaluation,"stores_evaluation.csv")


#stores_all %>% 
library(readr)
stores_validation <- read_csv("~/projects/m5/m5AV/stores_validation.csv")
stores_evaluation <- read_csv("stores_evaluation.csv")




rbind(stores_validation, stores_evaluation) %>% 
  write_csv("~/projects/m5/submission.csv")
