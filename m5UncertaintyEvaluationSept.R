...
calendar <- read_csv("m5-forecasting-uncertainty/calendar.csv")
...
library(readr)
sell_prices <- read_csv("m5-forecasting-uncertainty/sell_prices.csv", 
...
sales_train_evaluation <- read_csv("m5-forecasting-uncertainty/sales_train_evaluation.csv", 
...
#View(sales_train_evaluation)
# CA ----------------------------------------------------------------------

####### take CA state data, group by store_id, group by categogy

CAData <- sales_train_evaluation %>% filter(state_id == "CA") %>% 
  select(-state_id)
head(CAData[1:5])

#hts for CA
#cleanCASales <- function(df){

#  df$cat_id %<>% forcats::fct_relabel(~ str_sub(.x, 1,3))
...
#CASales <- cleanCASales(CAData) %>% mutate(
#  groupS = paste(store_id, cat_id, dept_id, item_id, sep = "")  
#) %>% select(-store_id, -cat_id, -dept_id, -item_id, -id) 
#head(CASales)
#praise::praise()
#transpose CASales to have each product per column
#CASales2 <- t(CASales[,-1942])
#colnames(CASales2) <- c(CASales$groupS)
#CASales2 <- as_tibble(CASales2)
#hts CASales2
#library(hts)
#CAx1 <- hts(CASales2, characters = c(4, 3, 6, 10 ) )#removed2cozsort hier by state

#h <- 28
#CAx11 <- aggts(CAx1, levels = c(0,1,2))
#n <- nrow(CAx11)
#p <- ncol(CAx11)
#CAxf <- matrix(NA, nrow = h, ncol = p)
#CAres <- matrix(NA, nrow = n, ncol = p)
#
#for(i in 1:p)
#{
#  CAfit <- auto.arima(CAx11[, i] ,xreg = as.matrix(kk5[,i]) ,stepwise = F, approximation = T,
#                     max.p = 15, max.q = 15, max.P = 12,
#                     max.Q = 12, max.order = 45,
#                     allowmean = T, lambda = "auto", 
#                     parallel = T  )
#  CAxf[, i] <- forecast(CAfit, h = h, xreg = as.matrix(kk6[,i]))$mean
#  CAres[, i] <- na.omit(CAx11[, i] - fitted(CAfit))
#}
#CAxf1 <- ts(CAxf, start = 1)
#CA.f <- MinT(CAxf1, get_nodes(CAx1), residual = res1, covariance = "sam", 
#            keep = "all", algorithms = "lu")
#colnames(CA.f) <- colnames(CAx11)
#write_csv(as.data.frame(CA.f), "CASales.csv") 
#ggsave("CAx1.pdf",plot = plot(CAx1), width = 65, height = 200, unit = "cm", dpi = 750,
#      limitsize = FALSE)
#accuracy.gts(forSalesCA, CAx1Test)#
#write.table(accuracy.gts(forSalesCA, CAx1Test), file = "accCombETS.txt")
...
CADataCA_1HobbiesCleanT <- t(CADataCA_1HobbiesClean[,-1942])
...
cal_CA_sell_CA_1_clean_train_reg <- cal_CA_sell_CA_1_clean[1:1941, ]
cal_CA_sell_CA_1_clean_forcast_reg <- cal_CA_sell_CA_1_clean[1942:1969,]
...
Sys.time()
for (level in c(0.5, 2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  for(i in 1:p)
  { 
    
    a <- CADataCA_1HobbiesCleanTT_hts_aggts[,i]
    b
    c
  ...
  colnames(CADataCA_1HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_1HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_1HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
...
CADataCA_1HouseholdCleanT <- t(CADataCA_1HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  dtlevel <- level/100
  dtnames <- colnames(CADataCA_1HouseholdCleanTT_hts_aggts)
  colnames(CADataCA_1HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_1HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_1HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
CADataCA_1FoodCleanT <- t(CADataCA_1FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  dtlevel <- level/100
  dtnames <- colnames(CADataCA_1FoodCleanTT_hts_aggts)
  colnames(CADataCA_1FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_1FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_1FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_2HobbiesCleanT <- t(CADataCA_2HobbiesClean[,-1942])
...
cal_CA_sell_CA_2_clean_train_reg <- cal_CA_sell_CA_2_clean[1:1941, ]
cal_CA_sell_CA_2_clean_forcast_reg <- cal_CA_sell_CA_2_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  dtlevel <- level/100
  dtnames <- colnames(CADataCA_2HobbiesCleanTT_hts_aggts)
  colnames(CADataCA_2HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_2HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_2HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_2HouseholdCleanT <- t(CADataCA_2HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_2HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_2HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_2HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_2FoodCleanT <- t(CADataCA_2FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_2FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_2FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_2FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_3HobbiesCleanT <- t(CADataCA_3HobbiesClean[,-1942])
...
cal_CA_sell_CA_3_clean_train_reg <- cal_CA_sell_CA_3_clean[1:1941, ]
cal_CA_sell_CA_3_clean_forcast_reg <- cal_CA_sell_CA_3_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_3HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_3HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_3HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_3HouseholdCleanT <- t(CADataCA_3HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_3HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_3HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_3HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_3FoodCleanT <- t(CADataCA_3FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_3FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_3FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_3FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_4HobbiesCleanT <- t(CADataCA_4HobbiesClean[,-1942])
...
cal_CA_sell_CA_4_clean_train_reg <- cal_CA_sell_CA_4_clean[1:1941, ]
cal_CA_sell_CA_4_clean_forcast_reg <- cal_CA_sell_CA_4_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_4HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_4HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_4HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_4HouseholdCleanT <- t(CADataCA_4HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_4HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_4HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_4HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
CADataCA_4FoodCleanT <- t(CADataCA_4FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(CADataCA_4FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(CADataCA_4FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/CADataCA_4FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXData <- sales_train_evaluation %>% filter(state_id == "TX") %>% 
  select(-state_id)
head(TXData[1:5])
...
#TXSales <- cleanTXSales(TXData) %>% mutate(
#  groupS = paste(store_id, cat_id, dept_id, item_id, sep = "")  
#) %>% select(-store_id, -cat_id, -dept_id, -item_id, -id) 

#TXSales2 <- t(TXSales[,-1942])  #check dim and rownames and colnames
#colnames(TXSales2) <- as.list(TXSales$groupS)
#library(hts)
#TXx1 <- hts(TXSales2, characters = c(4, 3, 6, 10 ) )#removed 2 coz we sort hier by state

#ggsave("TXx1.pdf",plot = plot(TXx1), width = 65, height = 200, unit = "cm", dpi = 750,
#      limitsize = FALSE)
#TXx1Data <- window(TXx1, start = 1, end = 1435, frequency = 1)
#TXx1Test <- window(TXx1, start = 1436)

#forSalesTX <- forecast.gts(TXx1Data, h = 478, method = "comb",fmethod = "arima", 
#                          weights = "mint", covariance = "shr",#lambda = 0, 
#                         parallel = TRUE, num.cores = 2)
#
#accuracy.gts(forSalesTX, TXx1Test)
...
TXDataTX_1HobbiesCleanT <- t(TXDataTX_1HobbiesClean[,-1942])
...
cal_TX_sell_TX_1_clean_train_reg <- cal_TX_sell_TX_1_clean[1:1941, ]
cal_TX_sell_TX_1_clean_forcast_reg <- cal_TX_sell_TX_1_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_1HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_1HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_1HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXDataTX_1HouseholdCleanT <- t(TXDataTX_1HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_1HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_1HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_1HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXDataTX_1FoodCleanT <- t(TXDataTX_1FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_1FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_1FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_1FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXDataTX_2HobbiesCleanT <- t(TXDataTX_2HobbiesClean[,-1942])
...
cal_TX_sell_TX_2_clean_train_reg <- cal_TX_sell_TX_2_clean[1:1941, ]
cal_TX_sell_TX_2_clean_forcast_reg <- cal_TX_sell_TX_2_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_2HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_2HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_2HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
TXDataTX_2HouseholdCleanT <- t(TXDataTX_2HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_2HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_2HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_2HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXDataTX_2FoodCleanT <- t(TXDataTX_2FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_2FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_2FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_2FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
TXDataTX_3HobbiesCleanT <- t(TXDataTX_3HobbiesClean[,-1942])
...
cal_TX_sell_TX_3_clean_train_reg <- cal_TX_sell_TX_3_clean[1:1941, ]
cal_TX_sell_TX_3_clean_forcast_reg <- cal_TX_sell_TX_3_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(TXDataTX_3HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_3HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_3HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXDataTX_3HouseholdCleanT <- t(TXDataTX_3HouseholdClean[,-1942])
...
Sys.time()
for (level in c(0.5,2.5, 16.5,  25, 50, 75, 83.5, 97.5, 
                99.5)){
  for(i in 1:p)
  { 
  ...
  colnames(TXDataTX_3HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_3HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_3HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
TXDataTX_3FoodCleanT <- t(TXDataTX_3FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  { 
    ...
    l <- stringr::str_which(
      TXDataTX_3FoodCleanTT_hts_aggts[,i], 
      as.character(
        TXDataTX_3FoodCleanTT_hts_aggts[,i][cumsum(TXDataTX_3FoodCleanTT_hts_aggts[,i]) != 0][1]
      ))[1] 
    #l <- match(
    # grep(
  ...
  colnames(TXDataTX_3FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(TXDataTX_3FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/TXDataTX_3FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
WIData <- sales_train_evaluation %>% filter(state_id == "WI") %>% 
  select(-state_id)
head(WIData[1:5])
...
#WISales <- cleanTXSales(WIData) %>% mutate(
#  groupS = paste(store_id, cat_id, dept_id, item_id, sep = "")  
#) %>% select(-store_id, -cat_id, -dept_id, -item_id, -id) 

#WISales2 <- t(WISales[,-1942])  #check dim and rownames and colnames
#colnames(WISales2) <- as.list(WISales$groupS)
#library(hts)
#WIx1 <- hts(WISales2, characters = c(4, 3, 6, 10 ) )#removed 2 coz we sort hier by state
#ggsave("WIx1.pdf",plot = plot(WIx1), width = 65, height = 200, unit = "cm", dpi = 750,
#      limitsize = FALSE)
#library(parallel)
#WIx1Data <- window(WIx1, start = 1, end = 1435, frequency = 1)
#WIx1Test <- window(WIx1, start = 1436)
#forSalesWI <- forecast.gts(WIx1Data, h = 478, method = "comb",fmethod = "arima", 
#                          weights = "mint", covariance = "shr",#lambda = 0, 
#                         parallel = TRUE, num.cores = 2)
#accuracy.gts(forSalesWI, WIx1Test)
...
WIDataWI_1HobbiesCleanT <- t(WIDataWI_1HobbiesClean[,-1942])
...
cal_WI_sell_WI_1_clean_train_reg <- cal_WI_sell_WI_1_clean[1:1941, ]
cal_WI_sell_WI_1_clean_forcast_reg <- cal_WI_sell_WI_1_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_1HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_1HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_1HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
WIDataWI_1HouseholdCleanT <- t(WIDataWI_1HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_1HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_1HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_1HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
WIDataWI_1FoodCleanT <- t(WIDataWI_1FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_1FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_1FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_1FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
WIDataWI_2HobbiesCleanT <- t(WIDataWI_2HobbiesClean[,-1942])
...
cal_WI_sell_WI_2_clean_train_reg <- cal_WI_sell_WI_2_clean[1:1941, ]
cal_WI_sell_WI_2_clean_forcast_reg <- cal_WI_sell_WI_2_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_2HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_2HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_2HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
WIDataWI_2HouseholdCleanT <- t(WIDataWI_2HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_2HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_2HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_2HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
WIDataWI_2FoodCleanT <- t(WIDataWI_2FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  write_csv(as.data.frame(WIDataWI_2FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_2FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
WIDataWI_3HobbiesCleanT <- t(WIDataWI_3HobbiesClean[,-1942])
...
cal_WI_sell_WI_3_clean_train_reg <- cal_WI_sell_WI_3_clean[1:1941, ]
cal_WI_sell_WI_3_clean_forcast_reg <- cal_WI_sell_WI_3_clean[1942:1969,]
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_3HobbiesCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_3HobbiesCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_3HobbiesCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time()
...
WIDataWI_3HouseholdCleanT <- t(WIDataWI_3HouseholdClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_3HouseholdCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_3HouseholdCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_3HouseholdCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
...
WIDataWI_3FoodCleanT <- t(WIDataWI_3FoodClean[,-1942])
...
for (level in c(0.5,2.5, 16.5, 25, 50, 75, 83.5, 97.5, 99.5)){
  ...
  colnames(WIDataWI_3FoodCleanTT_hts_aggts_forecast_ts_f) <- stringr::str_c(dtnames, dtlevel, sep = "_")
  
  write_csv(as.data.frame(WIDataWI_3FoodCleanTT_hts_aggts_forecast_ts_f), 
            file.path(glue::glue("m5UE/WIDataWI_3FoodCleanTT_hts_aggts_forecast_ts_f_{level}.csv") ) 
  ) 
}
Sys.time() 
